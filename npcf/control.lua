	...
	getControl = function(npc)
		...
		if npc._control then
			...
			control.walk_param = {
				...
				search_usable_destination = true
			}
		end
		if control._step_init_done ~= true then
			...
			control.real_speed = math.sqrt( math.pow(control.velocity.x,2) + math.pow(control.velocity.z,2))
		end
		return control
	end
}
...
function control_proto:walk(pos, speed)
	...
	if self.walk_param.find_path == true and vector.distance(self.pos, pos) < self.walk_param.max_find_path_distance then
		...
		local destpos
		if self.search_usable_destination then
			local rpos = vector.round(pos)
			for y = rpos.y+2, rpos.y-2, -1 do
				for x = rpos.x-2, rpos.x+2 do
					for z = rpos.z-2, rpos.z+2 do
						local p = {x=x, y=y, z=z}
		...
		if not destpos then
			...
			print("selected:", minetest.pos_to_string(destpos), "for", minetest.pos_to_string(self.pos))
		end
		local path = minetest.find_path(startpos, destpos, 10, 1, 5, "Dijkstra")
		self.path = path
		if not self.path and self.walk_param.direct_way == true then
			self.path = { pos }
			print("fallback path")
	...
	if self.walk_param.direct_way == true and self.path == nil then
		self.path = { pos }
...
function control_proto:_do_control_step(dtime)
	...
	if self._speed_bak > 0 and self.real_speed < 0.1 then
		print("Stuck!")
		self:stay()
	end
	...
	-- check/set yaw
	if self.path[1] and self.speed > 0 then
	...
	elseif minetest.registered_nodes[node[-1].name].walkable ~= false then
